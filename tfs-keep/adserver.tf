...
resource "openstack_blockstorage_volume_v3" "demo-adserver-sdb" {
  name        = "${var.prefix}p-demo-adserver-sdb"
  size        = 51
  metadata = {
    User = var.openstack_username
  }
}
resource "openstack_blockstorage_volume_v3" "demo-adserver-sdc" {
  name        = "${var.prefix}-demo-adserver-sdc"
  size        = 51
  metadata = {
    User = var.openstack_username
  }
...
resource "openstack_compute_instance_v2" "demo-adserver" {
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-adserver-sdb.id}"
    source_type           = "volume"
    boot_index            = 1
    destination_type      = "volume"
    delete_on_termination = true
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-adserver-sdc.id}"
    source_type           = "volume"
    boot_index            = 2
    destination_type      = "volume"
    delete_on_termination = true
