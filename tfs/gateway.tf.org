...
resource "openstack_blockstorage_volume_v3" "demo-gateway-sda" {
  name        = "${var.prefix}-demo-gateway-sda"
  size        = 51
...
resource "openstack_blockstorage_volume_v3" "demo-gateway-sdb" {
  name        = "${var.prefix}p-demo-gateway-sdb"
  size        = 51
  metadata = {
    User = var.openstack_username
  }
}
resource "openstack_blockstorage_volume_v3" "demo-gateway-sdc" {
  name        = "${var.prefix}-demo-gateway-sdc"
  size        = 51
  metadata = {
    User = var.openstack_username
  }
...
resource "openstack_compute_instance_v2" "demo-gateway" {
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-gateway-sda.id}"
    source_type           = "volume"
    boot_index            = 0
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-gateway-sdb.id}"
    source_type           = "volume"
    boot_index            = 1
    destination_type      = "volume"
    delete_on_termination = true
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-gateway-sdc.id}"
    source_type           = "volume"
    boot_index            = 2
    destination_type      = "volume"
    delete_on_termination = true
