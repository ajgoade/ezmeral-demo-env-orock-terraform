...
resource "openstack_blockstorage_volume_v3" "demo-externaldf-sdc" {
  count       = var.count-externaldf-hosts
  name        = "${var.prefix}-demo-externaldf-host-${count.index + 1}-sdc"
  size        = 51
  metadata = {
    User = var.openstack_username
  }
...
resource "openstack_compute_instance_v2" "demo-externaldf" {
  ...
  block_device {
    ...
    uuid                  = "${openstack_blockstorage_volume_v3.demo-externaldf-sdc.*.id[count.index]}"
    source_type           = "volume"
    boot_index            = 2
    destination_type      = "volume"
    delete_on_termination = true
